    ...
    return(
        <header className={`z-10 w-full h-[7em]  m-0 flex md:items-center justify-between fixed ${isTop ? "" : "shadow-lg"} ${isTop ? "" : "bg-[#fffafaf7]"}`}>
            ...
            <nav className="flex flex-col gap-5 pr-12">
                <button onClick={toggleMenu} className="relative z-10 top-9 left-5 p-2 text-[#FF4649] rounded-full text-xl hover:text-[#D40003] md:hidden"><RxHamburgerMenu /></button>
